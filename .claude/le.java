--- Minecraft 1.20.1 ---
Source:  net/minecraft/entity/LivingEntity.java
Jar:     /c/Users/huhai/Desktop/Java-mcmod/dragonkiller-template-1.20.1/.gradle/loom-cache/minecraftMaven/net/minecraft/minecraft-common-7a3f71a91a/1.20.1-net.fabricmc.yarn.1_20_1.1.20.1+build.10-v2/minecraft-common-7a3f71a91a-1.20.1-net.fabricmc.yarn.1_20_1.1.20.1+build.10-v2-sources.jar
...
public abstract class LivingEntity extends Entity implements Attackable {
	...
	protected void fall(double heightDifference, boolean onGround, BlockState state, BlockPos landedPosition) {
		...
		if (!this.getWorld().isClient && this.fallDistance > 3.0F && onGround && !state.isAir()) {
			...
			if (landedPosition.getX() != blockPos.getX() || landedPosition.getZ() != blockPos.getZ()) {
				double g = d - landedPosition.getX() - 0.5;
				double h = f - landedPosition.getZ() - 0.5;
				double i = Math.max(Math.abs(g), Math.abs(h));
				d = landedPosition.getX() + 0.5 + g / i * 0.5;
				f = landedPosition.getZ() + 0.5 + h / i * 0.5;
			}

			float j = MathHelper.ceil(this.fallDistance - 3.0F);
			double k = Math.min(0.2F + j / 15.0F, 2.5);
			int l = (int)(150.0 * k);
		...
		if (!this.getLandingBlockState().isAir()) {
			...
			if (i > 0 && this.isOnSoulSpeedBlock()) {
				...
				entityAttributeInstance.addTemporaryModifier(
					new EntityAttributeModifier(SOUL_SPEED_BOOST_ID, "Soul speed boost", 0.03F * (1.0F + i * 0.35F), EntityAttributeModifier.Operation.ADDITION)
				);
	...
	protected void addPowderSnowSlowIfNeeded() {
		if (!this.getLandingBlockState().isAir()) {
			...
			if (i > 0) {
				...
				entityAttributeInstance.addTemporaryModifier(
					new EntityAttributeModifier(POWDER_SNOW_SLOW_ID, "Powder snow slow", f, EntityAttributeModifier.Operation.ADDITION)
				);
			}
		}
	}
	...
	public void travel(Vec3d movementInput) {
		if (this.isLogicalSideForUpdatingMovement()) {
			...
			if (this.isTouchingWater() && this.shouldSwimInFluids() && !this.canWalkOnFluid(fluidState)) {
				...
				if (f < 0.0F && i > 0.0) {
					double m = j * -MathHelper.sin(f) * 0.04;
				...
				if (this.hasNoDrag()) {
					...
					this.setVelocity(vec3d6.x * f, q * 0.98F, vec3d6.z * f);
				}
			}
		}
	...
	protected float turnHead(float bodyRotation, float headRotation) {
		...
		if (Math.abs(g) > 50.0F) {
			this.bodyYaw = this.bodyYaw + (g - MathHelper.sign(g) * 50);
		}
	...
	public void tickMovement() {
		...
		if (this.bodyTrackingIncrements > 0) {
			...
			double g = MathHelper.wrapDegrees(this.serverYaw - this.getYaw());
			this.setYaw(this.getYaw() + (float)g / this.bodyTrackingIncrements);
	...
	public Box getBoundingBox(EntityPose pose) {
		...
		return new Box(
			-entityDimensions.width / 2.0F, 0.0, -entityDimensions.width / 2.0F, entityDimensions.width / 2.0F, entityDimensions.height, entityDimensions.width / 2.0F
		);
	}
