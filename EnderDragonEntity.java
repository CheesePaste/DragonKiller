--- Minecraft 1.20.1 ---
Source:  net/minecraft/entity/boss/dragon/EnderDragonEntity.java
Jar:     /mnt/d/Projects/MCMOD/DragonKiller/.gradle/loom-cache/minecraftMaven/net/minecraft/minecraft-common-7a3f71a91a/1.20.1-net.fabricmc.yarn.1_20_1.1.20.1+build.10-v2/minecraft-common-7a3f71a91a-1.20.1-net.fabricmc.yarn.1_20_1.1.20.1+build.10-v2-sources.jar
...
public class EnderDragonEntity extends MobEntity implements Monster {
			...
			if (this.isAiDisabled()) {
				...
				if (this.getWorld().isClient) {
					if (this.bodyTrackingIncrements > 0) {
						...
						double j = this.getZ() + (this.serverZ - this.getZ()) / this.bodyTrackingIncrements;
						double k = MathHelper.wrapDegrees(this.serverYaw - this.getYaw());
						this.setYaw(this.getYaw() + (float)k / this.bodyTrackingIncrements);
				...
				for (int ac = 0; ac < 3; ac++) {
					...
					float q = 1.5F;
					float ae = (ac + 1) * 2.0F;
					this.movePart(enderDragonPart, -(x * 1.5F + o * ae) * u, es[1] - ds[1] - (ae + 1.5F) * v + 1.5, (y * 1.5F + p * ae) * u);
				}
	...
	protected void updatePostDeath() {
		...
		if (this.getWorld() instanceof ServerWorld) {
			if (this.ticksSinceDeath > 150 && this.ticksSinceDeath % 5 == 0 && bl) {
				ExperienceOrbEntity.spawn((ServerWorld)this.getWorld(), this.getPos(), MathHelper.floor(i * 0.08F));
			}
		...
		if (this.ticksSinceDeath == 200 && this.getWorld() instanceof ServerWorld) {
			if (bl) {
				ExperienceOrbEntity.spawn((ServerWorld)this.getWorld(), this.getPos(), MathHelper.floor(i * 0.2F));
			}
	...
	public void crystalDestroyed(EndCrystalEntity endCrystal, BlockPos pos, DamageSource source) {
		...
		if (source.getAttacker() instanceof PlayerEntity) {
			...
			playerEntity = this.getWorld().getClosestPlayer(CLOSE_PLAYER_PREDICATE, pos.getX(), pos.getY(), pos.getZ());
		}
